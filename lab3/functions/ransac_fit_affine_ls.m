function [A,t] = ransac_fit_affine_ls(pts, pts_tilde, threshold)
    ...
    while k < k_max
        ...
        % Extract the points for the inliers
        inliers_pts = pts(:,ind);
        inliers_pts_tilde = pts_tilde(:,ind);

        % Refine the estimation with LS on the inliers    
        [A_sample, t_sample] = least_squares_affine(inliers_pts,inliers_pts_tilde);
    ...
    if success
        fprintf('======================================\n')
        fprintf('RANSAC with LS iterations done!\nIt took k=%.0f iterations to terminate...\nResidual: %.2f \n', k,best_res)
